...
// ---------------------------------------------------------------------------
// 株価（Twelve Data quote）と時間別天気（Open-Meteo hourly）— portrait ビュー用
// ---------------------------------------------------------------------------

/**
 * Twelve Data の quote オブジェクト（{ close, percent_change, status, ... }）から
 * { price: 現在値, chg: 前日比% } を返す。エラー/欠損時は null。
 */
function parseTwelveQuote(q) {
  if (!q || typeof q !== "object") return null;
  if (q.status === "error") return null;
  const price = Number(q.close);
  if (!isFinite(price)) return null;
  let chg = Number(q.percent_change);
  if (!isFinite(chg)) {
    // percent_change が無い場合は previous_close から算出。
    const prev = Number(q.previous_close);
    chg = isFinite(prev) && prev ? ((price - prev) / prev) * 100 : 0;
  }
  return {
...
/**
 * 銘柄を取引所（exchange / mic / country）ごとにグループ化する。
 * Twelve Data の quote バッチは取引所指定が全銘柄共通のため、混在時はリクエストを分ける。
 * 戻り値: [{ exchange, mic, country, items: [銘柄...] }]（config の並び順を保つ）。
 */
function groupStocksByExchange(symbols) {
  const groups = [];
  const index = new Map();
  for (const s of symbols || []) {
    ...
    const key = `${exchange}|${mic}|${country}`;
    let g = index.get(key);
    if (!g) {
      g = { exchange, mic, country, items: [] };
      index.set(key, g);
      groups.push(g);
    }
    g.items.push(s);
